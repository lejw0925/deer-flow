...
.PHONY: help config config-upgrade check check-agent-guidance install extension-install extension-list extension-enable extension-disable extension-remove setup doctor support-bundle detect-thread-boundaries detect-blocking-io upstream-check dev dev-daemon start start-daemon nginx stop up down clean docker-init docker-start docker-stop docker-logs docker-logs-frontend docker-logs-gateway docker-logs-redis
...
help:
	...
	@echo "  make upstream-check - Preflight the upstream merge in an isolated worktree"
...
# Preflight origin/main into downstream/main without changing this worktree.
upstream-check:
	@$(RUN_WITH_GIT_BASH) ./scripts/sync-upstream-check.sh
